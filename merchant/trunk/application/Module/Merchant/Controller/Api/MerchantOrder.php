...
class MerchantOrder extends AppApi {
    ...
    public function getOrderDetail($args = array()) {
        ...
        $items = $this->model->getOrderDetail($ids);
    ...
    public function add($args = array()) {
        ...
        $date = mktime($shipping['shippingHour'], 0, 0, $month, $day, $year);
        unset($data['date']);
        ...
        $result = $this->call(
            ...
            array(
                'id' => $carIds,
                'fields' => 'id,merchant_id,diner_name'
            )
        );
        $cars = $result['content'];
        if (! $cars) {
        ...
        // 创建订单
        $result = $this->model->add($item->merchant_id, $list, $date);
        if ($result === false) {
            return $this->export(array(
                'code' => 500,
                'message' => $this->db->sth->errorInfo()
    ...
    public function pay() {
        ...
        if ($orders) {
            foreach ($orders as $v) {
                if ($v->merchant_id != $this->user->id || $v->status != 0) continue;
                $orderIds[] = $v->id;
                $total += $v->total_price;
            }
        }
